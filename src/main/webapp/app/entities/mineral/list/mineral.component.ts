...
import { EntityArrayResponseType, MineralService } from '../service/mineral.service';
...
export class MineralComponent implements OnInit {
  ...
  trackId = (_index: number, item: IMineral): number => this.mineralService.getMineralIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.minerals = this.refineData(dataFromBody);
  }

  protected refineData(data: IMineral[]): IMineral[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IMineral[] | null): IMineral[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.mineralService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
